...
{public static void onLootTablesLoaded(LootTableLoadEvent event) {
        if (event.getName().equals(LootTableList.ENTITIES_ENDERMAN)) {
            LootPool table = new LootPool(
                    new LootEntry[]{
                            new LootEntryItem(ItemsOT.SHARD_END, 1, 1, new LootFunction[0],
                                    new LootCondition[]{new KilledByPlayer(false)},
                                    Reference.MOD_ID + ":ender_shard")},
                    new LootCondition[]{new RandomChance(0.0065F)}, new RandomValueRange(1),
                    new RandomValueRange(0), Reference.MOD_ID + "_enderman");
            event.getTable().addPool(table);
        }
            if (event.getName().equals(LootTableList.ENTITIES_ENDERMITE)) {
                LootPool table = new LootPool(
                        new LootEntry[]{
                                new LootEntryItem(ItemsOT.SHARD_END, 1, 1, new LootFunction[0],
                                        new LootCondition[]{new KilledByPlayer(false)},
        ...
        if (event.getName().equals(LootTableList.ENTITIES_ZOMBIE_PIGMAN)) {
            LootPool table = new LootPool(
                    new LootEntry[]{
                            new LootEntryItem(ItemsOT.SHARD_NETHER, 1, 1, new LootFunction[0],
                                    new LootCondition[]{new KilledByPlayer(false)},
        ...
        if (event.getName().equals(LootTableList.ENTITIES_BLAZE)) {
            LootPool table = new LootPool(
                    ...
                    new LootCondition[]{new RandomChance(100F)}, new RandomValueRange(1),
